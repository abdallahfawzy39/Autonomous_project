...
U = [[0.2],[0]]					##Control input matrix U (2x1) 0.2 velocity 0 angular
...
def callback(position_Noise_msg):
		global P, Q, R, U, X  
		global A, B, C
		global Flag_Initial, Control_Input, X_updated, P_updated


	  	# Extract the x,y and theta from noise node
		x = round(position_Noise_msg.data[0], 4)
		y = round(position_Noise_msg.data[1], 4)
		Theta = round(position_Noise_msg.data[2], 4)	
		...
		if Flag_Initial == 1:
			...
			P = P_updated
			Z = position_Noise_msg.data[0] #Set the sensor reading for the x position of the robot
		...
		XNoisy.append(position_Noise_msg.data[0]) 			#Array to hold noisy value of x coordinates
		YNoisy.append(position_Noise_msg.data[1]) 			#Array to hold noisy value of y coordinates
		ThetaNoisy.append(position_Noise_msg.data[2])		#Array to hold noisy value of theta coordinates
		...
		Control_Input.linear.x = 0.2 #linear velocity 0.2
...
### Initialize ROS Node 
	...
	sub = rospy.Subscriber('/Noise_data', Float32MultiArray, callback) #Subscribe to the  topic "/odom" of type "Odometry"
	pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10) #publish velocity to turtlebot on gazebo
	rospy.spin()

##Plotting of signals from sensor and noisy signals, model
	plt.figure(1)
